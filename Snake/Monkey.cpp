...
void Monkey::control_robot(Creature creature, float time, float dt, float alpha)
{
    for(int k=0; k<MUSCLE_NUM; k++)
    {
        creature.set_joint_velocity(k, SPEED_K*get_genome().calculate_torque(k, time), 0, dt, alpha);
    }
}
